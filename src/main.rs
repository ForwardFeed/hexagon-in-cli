...
fn grid_test() -> Result<(), hexagon_in_cli::grid::HexagonGridError>{
    ...
    grid.add_block(hexagon_in_cli::hexblock_q!(2,1, "2-2", "2-1"))?;
    grid.add_block_hex_c(3, 4)?;
    grid.add_block(hexagon_in_cli::hexblock_h!(2,1,"2-1", "2-0"))?
        .add_block(hexagon_in_cli::hexblock_h!(3,3, "aaaaaaaaaaaaaa", "4"))?;
    let neighbors  =
        grid
    ...
    let mut block = hexagon_in_cli::hexblock_q!(0,2, "12345", "1234567", "don't worry it's trimed automatically");
    block.text[2] = hexagon_in_cli::text::format_string("please worry it's NOT trimed automatically this way, you have to care for it yourself".to_string(), 7);
    grid.add_block(block)?;
    hexagon_in_cli::cli_draw::draw(&grid);
    Ok(())
}